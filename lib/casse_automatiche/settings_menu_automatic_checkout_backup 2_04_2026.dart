  ...
  Widget _buildActionsPanel(bool isDark, Color card, BuildContext context) {
    ...
    final actions = <_ActionDef>[
      ...
      _ActionDef(
        ...
        onTap: () {
          final m = context.read<ControllerAutomaticCheckout>().model;
          if (m == null) { SnackBarForcedClosure('Cassetto non selezionato', Colors.red); return; }
          if (m.contains('bs_cash')) { SnackBarForcedClosure('Funzione non disponibile per il cassetto selezionato', Colors.red); return; }
          withDrawCoins(context);
        },
      ),
      _ActionDef(
...
class _ActionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Material(
    ...
    elevation:    2,
    shadowColor:  def.color.withOpacity(0.2),
    child: InkWell(
      onTap: def.onTap,
      borderRadius: BorderRadius.circular(16),
      splashColor: def.color.withOpacity(0.12),
      child: Container(
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: def.color.withOpacity(isDark ? 0.35 : 0.15),
          ),
        ),
        padding: const EdgeInsets.all(18),
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: def.color.withOpacity(isDark ? 0.2 : 0.1),
                borderRadius: BorderRadius.circular(10),
              ),
