      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Text('Disabilita cassetti:',
                  style: const TextStyle(
                      fontSize: 12,
                      color: _kVerde,
                      fontWeight: FontWeight.w600)),
              Container(
                    child: Checkbox(value: ctrlDrawer._selectedDrawer == null, onChanged: (v){
...
class _DrawerTile extends StatelessWidget {
  ...
  IconData _iconForModel(String m) {
    ...
    if (m.contains('cpi'))       return Icons.money_rounded;
...
void changeCash(BuildContext context) async {
  ...
  final ctrlAutomaticCheckout  = context.read<ControllerAutomaticCheckout>();
  final modelAutomatickCeckout = ctrlAutomaticCheckout.model;
  if (modelAutomatickCeckout == 'cpi') {
    await performCpiRefund(context, ctrlAutomaticCheckout);
    return;
  }
...
void refill(BuildContext context) async {
  ...
  if (modelAutomatickCeckout == 'cpi') {
    await performCpiPayment(context, ctrlAutomaticCheckout);
    return;
  }
  int insert = 0;
    ...
    if (modelAutomatickCeckout == 'cashmatic') {
      ...
      cashmatic.launchOperationAndWait(
        ...
        if (finaldata['data'] != null) {
          ...
          LogService.instance().saveLog('Prelievo', 'Prelevati ${finaldata['data']['dispensed']}','');
...
// =====================================================================
// CPI CASH AUTOMATION INTEGRATION
// =====================================================================
...
class CpiApiService {
  ...
  Future<bool> authenticate(String ipAddress, String username, String password) async {
    ...
    try {
      final response = await http.post(
        ...
        body: 'grant_type=password&username=${Uri.encodeComponent(username)}&password=${Uri.encodeComponent(password)}',
  ...
  Future<String> startTransaction(String type, double amount, String currencyCode) async {
    final String apiTransactionType = type == 'Refund' ? 'Dispense' : type;
    final int valueInBaseUnits = (amount * 100).round();
...
class CpiSimpleTransactionDialog extends StatefulWidget {
  final String transactionType;
  final double amount;
  final String currencyCode;
  const CpiSimpleTransactionDialog({super.key, required this.transactionType, required this.amount, required this.currencyCode});
  @override
  State<CpiSimpleTransactionDialog> createState() => _CpiSimpleTransactionDialogState();
}

class _CpiSimpleTransactionDialogState extends State<CpiSimpleTransactionDialog> {
  ...
  void _startPolling() {
    _timer = Timer.periodic(const Duration(seconds: 1), (t) async {
      ...
      try {
        ...
        if (!mounted) return;
        setState(() {
          _accepted = rawAccepted / 100.0;
          _dispensed = rawDispensed / 100.0;
          _status = apiStatus.toString();
        });
        if (['CompletedSuccess', 'Cancelled', 'DeviceError', 'NotStartedBusy', 'NotStarted'].contains(apiStatus)) {
          t.cancel();
          if (mounted) setState(() => _finished = true);
        }
      } catch (e) {
        debugPrint('CPI polling error: $e');
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text('CPI - ${widget.transactionType}'),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text(_status, textAlign: TextAlign.center),
          const SizedBox(height: 16),
          if (widget.amount > 0) Text('Dovuto: ${widget.currencyCode} ${widget.amount.toStringAsFixed(2)}'),
          Text('Accettato: ${_accepted.toStringAsFixed(2)}'),
          Text('Erogato: ${_dispensed.toStringAsFixed(2)}'),
          if (_error != null) ...[
            const SizedBox(height: 8),
...
Future<void> performCpiPayment(BuildContext context, ControllerAutomaticCheckout ctrl) async {
  final drawer = ctrl.idSelectedDrawer;
  if (drawer == null) return;
  final ip = drawer.params['ip_address'] ?? 'localhost';
  final user = drawer.params['username'] ?? 'admin@cpi.com';
  final pass = drawer.params['password'] ?? 'Test0001';
  HttpOverrides.global = CpiHttpverrides();
  final ok = await cpiApi.authenticate(ip, user, pass);
  if (!ok) {
    SnackBarForcedClosure('Errore autenticazione CPI. Controllare IP / credenziali', Colors.red);
    return;
  }
  final amount = await numericKeyboard(context, 0, title: 'Importo CPI');
  if (amount == null || amount <= 0) return;
  await showDialog(
    context: context,
    barrierDismissible: false,
    builder: (c) => CpiSimpleTransactionDialog(
...
Future<void> performCpiRefund(BuildContext context, ControllerAutomaticCheckout ctrl) async {
  final drawer = ctrl.idSelectedDrawer;
  if (drawer == null) return;
  final ip = drawer.params['ip_address'] ?? 'localhost';
  final user = drawer.params['username'] ?? 'admin@cpi.com';
  ...
  final amount = await numericKeyboard(context, 0, title: 'Quanto vuoi prelevare?');
  if (amount == null || amount <= 0) return;
  await showDialog(
    context: context,
    barrierDismissible: false,
